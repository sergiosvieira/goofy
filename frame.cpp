...
Coordinates Frame::current(int a_cols)
{
    int sx = (m_current % a_cols) * m_width;
    int sy = (m_current / a_cols) * m_height;
    Coordinates result = {sx, sy};

    return result;
}

Frame::Frame(int a_first, int a_last, int a_current, int a_width, int a_height)
{
    m_first = a_first;
    m_last = a_last;
    m_current = a_current;
